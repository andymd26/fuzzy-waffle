...
To bridge the gap in data we supplement it with data from the California energy almanac.
In July of 2007 the California Energy Commission adopted the Quartery Fuel and Energy Report (QFER) requirement, which provide for the collection of information concerning electric generation, amongst other data \parencite{cec2016}. 
As a result, we have generator level, heat rate data for the period 2007 to present.  
Two additional sources of heat rate data are available but represent averages, as opposed to generator level data: the underlying assumptions of the Annual energy outlook (AEO) and the Electric Power Annual.\footnote{Archived AEO's and supplementary tables are available at  \url{https://www.eia.gov/forecasts/aeo/archive.cfm}.}
...
Usually, two heat rates are provided: the heat rate in that year and the nth of its kind heat rate. 
...
Finally, the Electric Power Annual provides an extremely aggregated heat rate statistic by fuel type for the period of the analysis. 
...
The heat rate data, which is at the generator level, is at a finer resolution than we have for input fuel prices.
For example, we only have one price for coal available in the Electric power monthly documentation, Electric Utility Receipts of and Average Cost for Fossil Fuels while EIA Form 860 has 10 possible coal primary fuel categories.   
While it may seem odd to discuss the fuel price data in the heat rate section, we can increase the sample size for the linear regression (and generally stop abusing our data) undertaken to predict missing heat rate values if we preprocess the data by aggregating the existing primary fuel categories to the available primary fuel price data.\footnote{We initially ran the regression for each unique prime mover, primary fuel pairing. However, for a number of pairings we don't have the degrees of freedom to have any certainty in the output. Through aggregation we can increase our sample size and address this issue.} 

The primary fuel source category has 60 unique values in the EIA Form 860 database.
However, the EIA tracks electric utility fuel purchases for a much smaller set of fuels including, coal, oil, and natural gas).
Historical fuel price information is available for kerosene, uranium, and propane.
...
We map the following primary fuel categories to natural gas: landfill gas (LFG)(3,268 MW, 2,436 units, 5 years old), municipal solid waste (MSW)(3,426.41 MW, 154 units, 15 years old), methane (MTE)(3.4 MW, 5 units, 0 years old), natural gas (NG)(835,301 MW, 11,235 units, 16 years old), other biomass gases (OBG)(466 MW, 286 units, 6.3 years old), synthetic natural gas (SNG)(360 MW, 4 units, 15 years old), synthetic gas other than coal derived (SG)(274 MW, 2 units, 37 years old), coal-derived synthetic gas (SGC)(1,110 MW, 7 units, 29 years old), and synthetic gas from petroleum coke (SGP)(274 MW, 2 units, 38 years old). 
...
We are unsure of how to map the following primary fuels to the available fuel price data: black liquor (BLQ)(6,570 MW, 306 units, 34 years old), liquified propane (LPG)(374.5 MW, 8 units, 19 years old), steam (STM)(61 MW, 3 units, 52 years old), waste heat (WH)(25,270 MW, 436, 16.5 years old), purchased steam (PUR)(651.8 MW, 33 units, 35 years old), sludge waste (SLW)(15 MW, 9 units, 10 years old), tires (TDF)(189 MW, 8 units, 9.5 years old), wood waste liquids (WDL)(125 MW, 8 units, 36 years old), blast furnace gas (BFG)(2,236 total MW, 65 units, 45 years old), other gas (OG)(3,694 MW, 178 units, 29 years old), and refinery gas (RG)(52.1 MW, 3 units, 10 years old). 
We don't list the previously listed, undefined categories. 
In many of these cases the primary fuel represents an extremely small portion of the total installed capacity in the data set. 

\subsection{Ignored categories}
We ignore the following primary fuel categories: biomass generic (BIO), gas generic (GAS), re refined motor oil (RRO), topped crude oil (TOP), petroleum generic (PET), no 3 fuel oil (FO3), crude oil (CRU), coal oil mixture (COM), propane (PRO), coal water mixture (CWM), plutonium (PL), thorium (TH), coke oven gas (COG), liquefied natural gas (LNG), methanol (MTH) because there are no units in the dataset using these fuels in the data set. We also ignore propane (PG)(2.5 MW, 25 units, 0 years old), which does have a very small amount of installed capacity. 
